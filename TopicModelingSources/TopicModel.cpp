...
TopicModel::TopicModel(size_t numberOfTopics, double alphaSum, double beta)
{
    ...
    for (size_t ind = 0; ind < numberOfTopics; ++ind)
    {
        alpha[ind] = alphaSum/numberOfTopics;
    }
...
void TopicModel::buildInitialTypeTopicCounts ()
{
    ...
    for (int type = 0; type < numTypes; ++type)
    {
        ...
        for (size_t position = 0; position < topicCounts.size() && topicCounts[position] > 0; ++position)
        {
...
void TopicModel::optimizeAlpha(std::vector<WorkerRunnable> runnables)
{
    ...
    for (size_t thread = 0; thread < numThreads; ++thread)
    {
        ...
        for (size_t topic=0; topic < numTopics; ++topic)
        {
            ...
            {
                for (size_t count=0; count < sourceTopicCounts[topic].size(); ++count)
                {
...
void TopicModel::fillTopicSortedWords()
{
    ...
    for (size_t type = 0; type < numTypes; ++type)
    {
        ...
        for (int index = 0; index < topicCounts.size() && topicCounts[index] > 0; ++index)
        {
